...
Lumen 默认在启动时从项目官方 GitHub Releases 检查新版本。用户可以在设置中关闭自动检查，也可以随时手动检查。更新包安装前会用同一版本附带的 `SHA256SUMS.txt` 校验完整性。
...
- 网络访问仅用于读取 `Bugliiiiii/lumen` 官方 GitHub Releases 版本信息和下载用户确认的更新包。
- 不包含遥测上报、远程桌面、视频传输或键鼠共享功能。

## 隐私

Lumen 没有分析追踪、账户体系或遥测上报。除 GitHub Releases 更新请求外，应用不发起其他网络通信。所有配置仅存储在当前用户的本地应用数据目录中。

Windows 发现更新后可直接下载安装并重启。macOS 会将校验后的 DMG 保存到“下载”目录并打开，由系统完成安装确认。
